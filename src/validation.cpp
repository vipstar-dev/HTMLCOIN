...
bool CheckInputScripts(const CTransaction& tx, TxValidationState &state, const CCoinsViewCache &inputs, unsigned int flags, bool cacheSigStore, bool cacheFullScriptStore, PrecomputedTransactionData& txdata, std::vector<CScriptCheck> *pvChecks) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (::ChainActive().Tip()->nHeight < Params().GetConsensus().QIP5Height && !CheckHash(tx, inputs)) {
        return(state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, error("CheckHash: Trying to spend locked outputs"), REJECT_INVALID, "bad-hash"));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!IsInitialBlockDownload() && !CheckSyncCheckpoint(block.GetHash(), pindex->nHeight))
        return state.Invalid(BlockValidationResult::BLOCK_CHECKPOINT, error("%s: Block rejected by synchronized checkpoint", __func__), REJECT_CHECKPOINT, "bad-block-checkpoint-sync");
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (!::ChainstateActive().IsInitialBlockDownload() && !CheckSyncCheckpoint(block.GetHash(), nHeight, pindexPrev)) {
        return state.Invalid(BlockValidationResult::BLOCK_CHECKPOINT, error("%s: rejected by checkpoint sync %s", __func__, block.GetHash().ToString()), REJECT_CHECKPOINT, "bad-block-checkpoint-sync");
    }
    ...
    if (block.GetBlockTime() <= pindexPrev->GetBlockTime() - 15 * 60)
        return state.Invalid(BlockValidationResult::BLOCK_TIME_PAST, "time-too-old", "block timestamp is too early");
